...
export const Timeline = ({ data, className }) => {
  ...
  return (
    <div
      className={cn(
        "w-full bg-white/50 dark:bg-slate-900/50 font-sans md:px-10 rounded-2xl overflow-hidden md:overflow-visible",
        className
      )}
      ref={containerRef}
    >
        ...
        {data.map((item, index) => (
          <div key={index} className={cn("flex justify-start pt-10 md:pt-10 md:gap-10")}>
            <div className={cn("sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full")}>
              <div className={cn("h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-gray-950 flex items-center justify-center")}>
                <div className={cn("h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2")} />
              </div>
            ...
            <div className={cn("relative pl-20 pr-4 md:pl-4 w-full")}>
              <h3 className={cn("md:hidden block text-base mb-4 text-left font-bold text-neutral-500 dark:text-neutral-500")}>
                {item.title}
              </h3>
              {item.content}
            </div>
